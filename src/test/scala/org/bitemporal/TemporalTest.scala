...
class TemporalTest extends FlatSpec with Matchers {
  ...
  val s1 : Temporal[Student, Int] = new Temporal[Student, Int](new Student("Some", "Body"), new Period(TestData.d1, TestData.d4))
  val s2 : Temporal[Student, Int] = new Temporal[Student, Int](new Student("Some", "Head"), new Period(TestData.d2, TestData.d3))

  // update one with the other
  val updated : List[Temporal[Student, Int]] = s1.update(s2)
